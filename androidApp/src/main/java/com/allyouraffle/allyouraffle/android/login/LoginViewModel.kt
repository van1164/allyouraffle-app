...
class LoginViewModel(
    ...
    private val _loginApiState = MutableStateFlow<Api.ApiState>(Api.ApiState.Before)
    val loginApiState = _loginApiState.asStateFlow()
    @SuppressLint("CommitPrefEdits")
    fun handleGoogleSignInResult(task: Task<GoogleSignInAccount>?) {
        if (task == null) {
            _loginApiState.value = Api.ApiState.Error
            return
        }
        try {
            _loginApiState.value = Api.ApiState.Loading
            ...
            account?.let {
                ...
                _loginApiState.value = Api.ApiState.Success
            } ?: run {
                _loginApiState.value = Api.ApiState.Error
            }
